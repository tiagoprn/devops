...
for i in *; do
    ...
    newname=$(echo $i | grep -o -m 1 -P -e "[0-9]+" | head -1 | xargs printf "%04.0f"); # -o is to just output the matched string, -m 1 is to stop at the first match, head -1 will ensure just the first match on the command output
