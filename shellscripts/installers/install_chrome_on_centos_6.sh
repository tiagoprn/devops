...
# gcc, rpm-build and rpmdevtools.
# The latest Google Chrome RPM if not already downloaded (or out-of-date).
# 4 RPM packages from Fedora 15 if not already downloaded.

# It then copies 9 libraries from the F15 packages into /opt/google/chrome/lib.
...
# before calling exec*() routines and then restores the env var afterwards.
# /opt/google/chrome/google-chrome is also modified to point LD_PRELOAD
# to the installed library. This is avoids having LD_LIBRARY_PATH set when
# sub-processes are run.
...
#   Bumped chrome-deps version to 1.01 because of the unset_vars.c change.
...
#   built unset_vars.so library and a script which is run post-install
...
#   specify once time.
# - Added libdl.so.2 to the Fedora library list (for unset_vars.so).
...
# Extract and install libraries from Fedora RPMs
{
   ...
   install_library glib2-2.28.8-1.fc$fedver       $rellib/libgio-2.0.so.0
   install_library glib2-2.28.8-1.fc$fedver       $rellib/libglib-2.0.so.0
   install_library glib2-2.28.8-1.fc$fedver       $rellib/libgobject-2.0.so.0
   install_library gdk-pixbuf2-2.23.3-2.fc$fedver $relusrlib/libgdk_pixbuf-2.0.so.0
}
...
- Updated unset_vars.c to fix Google Talk Plugin crash.
